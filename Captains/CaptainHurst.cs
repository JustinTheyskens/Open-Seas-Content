...
{
    public class CaptainHurst : BaseCreature
    {
        ...
        [Constructable]
        public CaptainHurst() : this(null) { }

        public CaptainHurst(BaseGalleon galleon)
            : this(galleon, AIType.AI_Melee, FightMode.Weakest, 10, 1, .2, .4)
        {
        }

        public CaptainHurst(BaseGalleon galleon, AIType ai, FightMode fm, int per, int range, double passive, double active)
            : base(ai, fm, per, range, passive, active)
        {
            ...
            //Title = "the legendary ghost pirate";
            Hue = 2448;
            BaseSoundID = 0x17E; // 0x482;
            Body = 1776;
            //Hidden = true;
            //Frozen = true;
            ...
            Timer.DelayCall(TimeSpan.FromSeconds(2.0), new TimerCallback(DoFog));
        }

        public void SpawnShip()
        {
            BaseGalleon gal = new OrcishGalleon(Direction.North);
        ...
        public void FillHold(BaseGalleon gal)
        {
            if (gal.GalleonHold != null)
            {
                Container hold = gal.GalleonHold;

                if (hold != null)
                {
                    hold.DropItem(new Gold(Utility.RandomMinMax(5000, 25000)));
                    hold.DropItem(new Copper(Utility.RandomMinMax(100, 250)));

                    int cnt = Utility.RandomMinMax(10, 20);

                    for (int i = 0; i < cnt; i++)
                    {
                        Item item = RunicReforging.GenerateRandomItem(gal);
                    ...
                    hold.DropItem(new Matches(Utility.RandomMinMax(100, 200)));
                    hold.DropItem(new HeavyCannonball(Utility.RandomMinMax(25, 75)));
                    hold.DropItem(new LightCannonball(Utility.RandomMinMax(25, 75)));
                    hold.DropItem(new HeavyGrapeshot(Utility.RandomMinMax(25, 75)));
                    hold.DropItem(new LightGrapeshot(Utility.RandomMinMax(25, 75)));
                    hold.DropItem(new HeavyPowderCharge(Utility.RandomMinMax(25, 75)));
                    hold.DropItem(new LightPowderCharge(Utility.RandomMinMax(25, 75)));
                    hold.DropItem(new FuseCord(Utility.RandomMinMax(25, 75)));
                    ...
                    if (Utility.RandomDouble() < 0.2)
                        hold.DropItem(new GhostShipAnchor()); // special artifact
                }
            }
        }
        ...
        public void DoFog()
        {
            ...
            if (m_Galleon != null)
                p = m_Galleon.Location;

            Timer.DelayCall(TimeSpan.FromSeconds(2.5), TimeSpan.FromSeconds(2.5), 6, () =>
            {
                for (int i = 0; i < 20; i++)
                {
                    Misc.Geometry.Circle2D(p, Map, i, (pnt, map) =>
                    {
                        Timer.DelayCall<int>(TimeSpan.FromMilliseconds(i * 150), index =>
                        {
                            Effects.SendLocationEffect(EffectItem.Create(pnt, map, EffectItem.DefaultDuration), Map, 0x3728, 10, 1, 0, 2);
                        }, i);
                    });
                }
            });
        ...
        public override void OnThink()
        {
            ...
            if (m_Galleon != null && !Ghostly)
            {
                if (m_Galleon.GalleonPilot != null)
                {
                    m_Galleon.GalleonPilot.Body = 26;
                    m_Galleon.GalleonPilot.Hue = 2448;
                    Ghostly = true;
                }
            }
        }
        ...
        public CaptainHurst(Serial serial)
            : base(serial)
        {
        }
