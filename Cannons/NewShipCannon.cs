...
namespace Server.Items
{
    ...
    */
    ...
    public abstract class NewShipCannon : Container, INewShipCannon
    {
        private int m_Hits;
        ...
        public abstract NewShipCannonDeed GetDeed { get; }
        ...
        public NewShipCannon(BaseShip ship)
            : base(0x2198)
        {
            Movable = false;
            Ship = ship;
        ...
        {
            if (Ship.GetSecurityLevel(from) >= SecurityLevel.Crewman)
            {
                ...
                AddAction(from, from.Alive && Ship.Contains(from) && from.InRange(Location, 2) ? 1149653 : 1149654); // You are now operating the cannon. : You are too far away.
        ...
        {
            if (BaseShip.CannonIDs[0].Any(id => id == ItemID))
            {
                return Direction.South;
            }
            if (BaseShip.CannonIDs[1].Any(id => id == ItemID))
            {
        ...
        {
            ShipAmmoInfo ammo = ShipAmmoInfo.GetAmmoInfo(AmmoType);
                    ...
                    case AmmunitionType.FlameCannonball:
                        {
                            ...
                            for (int i = -lateralOffset; i <= lateralOffset; i++)
                            {
                                ...
                                if (b != null && b != Ship && b.IsEnemy(Ship))
                                    list.Add(b);

                                BaseSeaFort f = FindSeaFortTarget(newPoint, map, ammo);
                            ...
                            if (list.Count > 0)
                            {
                                ...
                                else if (toHit is BaseSeaFort)
                                {
                                    Timer.DelayCall(delay, new TimerStateCallback(OnFortHit), new object[] { (BaseSeaFort)toHit, newPoint, ammo, shooter });
                                    hit = true;
                                }
                            }
                        }
        ...
        private BaseBoat FindValidBoatTarget(Point3D newPoint, Map map, ShipAmmoInfo info)
        {
        ...
        private BaseSeaFort FindSeaFortTarget(Point3D newPoint, Map map, ShipAmmoInfo info)
        {

            BaseSeaFort fort = null;
            SeaFortController controller = SeaFortController.FindControllerAt(newPoint, map);

            if (controller == null)
                return null;

            if (controller.SeaFort != null ) 
                fort = controller.SeaFort;
            else
                return null;

            // this part may not be needed
            if (fort != null && info.RequiresSurface)
            {
                if (newPoint.X <= fort.X - 2 || newPoint.X >= fort.X + 2)
                    return null;
        ...
        public virtual void OnShipHit(object obj)
        {
            ...
            ShipAmmoInfo ammoInfo = list[2] as ShipAmmoInfo;
            ...
            if (target != null && Ship != null)
            {
                ...
                if (ammoInfo.AmmoType == AmmunitionType.Grapeshot)
                {
                    ...
                    {
                        damage = (int)(ammoInfo.GetDamage(this) * Ship.CannonDamageMod);
                ...
                if (Ship.Map != null)
                {
                    IPooledEnumerable eable = Ship.Map.GetItemsInRange(hit, 1);

                    foreach (Item item in eable)
                    {
                        if (item is IShipCannon && !Ship.Contains(item))
                        {
        ...
        public virtual void OnFortHit(object obj)
        {
            object[] list = (object[])obj;
            BaseSeaFort target = list[0] as BaseSeaFort;
            Point3D pnt = (Point3D)list[1];
            ShipAmmoInfo ammoInfo = list[2] as ShipAmmoInfo;
            Mobile shooter = list[3] as Mobile;
            ...
            if (target != null && Ship != null)
            {
                ...
                //if (target.Controller != null) // maybe not even needed?
                    //z = target.Controller.Z;
                ...
                if (ammoInfo.AmmoType == AmmunitionType.Grapeshot)
                {
                    ...
                    if ( target.Controller != null)
                        newHit = target.Controller.Location;
                    else
                        newHit = target.Location;
                    ...
                    for (int count = 15; count > 0; count--)
                    {
                        ...
                        {
                            Point3D loc = new Point3D(newHit.X + Utility.RandomMinMax(-4, 4), newHit.Y + Utility.RandomMinMax(-4, 4), newHit.Z);
                            Effects.SendPacket(loc, target.Map, new GraphicalEffect(EffectType.FixedXYZ, Serial.Zero, Serial.Zero, 0x36CB, loc, loc, 15, 15, true, true));
                            target.OnTakenDamage(shooter, damage);
                            MobileOnBoardDamage(shooter, loc, ammoInfo);
                        });
                ...
                else
                {
                    ...
                    Effects.SendPacket(hit, target.Map, new GraphicalEffect(EffectType.FixedXYZ, Serial.Zero, Serial.Zero, 0x36CB, hit, hit, 15, 15, true, true));
                    target.OnTakenDamage(shooter, damage);
                    Point3D newHit = new Point3D(target.X + 1, target.Y, target.Z);
                    Effects.SendPacket(hit, target.Map, new GraphicalEffect(EffectType.FixedXYZ, Serial.Zero, Serial.Zero, 0x36CB, newHit, newHit, 15, 15, true, true));
                }
        ...
        public void MobileOnBoardDamage(Mobile shooter, Point3D pnt, ShipAmmoInfo info)
        {
            List<IDamageable> list = new List<IDamageable>();

            if (Map == null || Map == Map.Internal || Ship == null)
                return;
            ...
            foreach (IDamageable dam in eable.OfType<IDamageable>())
            {
                ...
                if (mob != null && (!shooter.CanBeHarmful(mob, false) || Ship.Contains(mob)))
                    continue;
        ...
        public virtual void OnMobileHit(object obj)
        {
            ...
            int damage = (int)(Utility.RandomMinMax(info.MinDamage, info.MaxDamage) * Ship.CannonDamageMod);
        ...
        public List<IDamageable> FindDamageables(Mobile shooter, Point3D pnt, Map map, bool player, bool pet, bool monsters, bool seacreature, bool items)
        {
            ...
            foreach (IDamageable dam in eable.OfType<IDamageable>())
            {
                ...
                if (items && dam is DamageableItem && ((DamageableItem)dam).CanDamage && !Ship.Contains(dam))
                    list.Add(dam);
        ...
        public override void GetContextMenuEntries(Mobile from, List<ContextMenuEntry> list)
        {
            ...
            if (Ship.GetSecurityLevel(from) >= SecurityLevel.Officer)
            {
        ...
        private class UnloadContext : ContextMenuEntry
        {
            ...
            public NewShipCannon Cannon { get; set; }

            public UnloadContext(NewShipCannon cannon, Mobile from)
        ...
        private class DismantleContext : ContextMenuEntry
        {
            ...
            public DismantleContext(NewShipCannon cannon, Mobile from)
                : base(1116069, 2)
            {
                From = from;
                Cannon = cannon;
            ...
            public override void OnClick()
            {
                NewShipCannonDeed deed = Cannon.GetDeed;
            ...
            {
                if (Cannon.Ship.GetSecurityLevel(From) != SecurityLevel.Captain)
                {
        ...
        private class RepairContext : ContextMenuEntry
        {
            ...
            public RepairContext(NewShipCannon cannon, Mobile from)
                : base(1116602, 2)
            {
                From = from;
                Cannon = cannon;
        ...
        public NewShipCannon(Serial serial) : base(serial) { }
        ...
        public class ShipCannonGump : BaseGump
        {
            ...
            public ShipCannonGump(PlayerMobile pm, NewShipCannon cannon)
                : base(pm, 100, 100)
            {
                Cannon = cannon;
            ...
            public override void AddGumpLayout()
            {
                ...
                AddHtml(50, 10, 230, 18, "<BASEFONT COLOR=#6082B6>Ship Cannon Control", false, false);
                ...
                AddHtmlLocalized(115, 71, 115, 18, 1114057, Cannon.Loaded == CannonAction.Finish ? ShipAmmoInfo.GetAmmoName(Cannon).ToString() : "#1149636", Cannon.Loaded == CannonAction.Finish ? 0x1FE7 : 0x7CE7, false, false); // Cannonball / Not Loaded
            ...
            {
                if (info.ButtonID == 0 || !Cannon.Ship.Contains(User))
                {
